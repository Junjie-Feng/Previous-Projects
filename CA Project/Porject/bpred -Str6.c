	     ...
	     unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  switch (class) {
  case BPredComb:
    /* Strategy 6 component */
    pred->dirpred.bimod = 
      bpred_dir_create(BPred1bit, bimod_size, 0, 0, 0);
    ...
    pred->dirpred.meta = 
      bpred_dir_create(BPred1bit, meta_size, 0, 0, 0);
  ...
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  case BPred1bit:/* Strategy 6*/
    if (!l1size || (l1size & (l1size-1)) != 0)
      fatal("1bit table size, `%d', must be non-zero and a power of two", 
	    l1size);
    ...
    /* initialize counters to weakly this-or-that */
    /* according to strategy 6, there is only taken or untaken. so initialized counter is 1 or 0*/
    flipflop = 1;
		 ...
		 md_addr_t baddr)		/* branch address */
{
  ...
  switch (pred_dir->class) {
	...
	int l1index, l2index;
      ...
      break;
    case BPred1bit:/*Strategy 6: char p stores 0 or 1 which indicates the prediction result is taken or untaken*/
					 ...
					 * used on mispredict recovery */
{
  ...
  if (pbtb == NULL)
    {
      /* Strategy 6: Since this strategy uses 1-bit predictor, so 0 means untaken, 1 means taken*/
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  if (dir_update_ptr->pdir1)
    {
	/* Strategy 6: it is similar to default bimod, so only the _pdir1 will be updated as 1-bit.*/
      if (taken)
	{
