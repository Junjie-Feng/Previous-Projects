...
struct bpred_t *			/* branch predictory instance */
	bpred_create(enum bpred_class class,	/* type of predictor to create */
		...
		unsigned int l1size2,	//*****************************************************
		unsigned int l2size2,
		unsigned int shift_width2,
		unsigned int xor2,
		...
		unsigned int retstack_size) /* num entries in ret-addr stack */
{
  ...
  case BPredComb:
    //****************************************
	  pred->dirpred.twolev2 =
		  bpred_dir_create(BPred2Level2, l1size2, l2size2, shift_width2, xor2);
  ...
  case BPred2Level2:
	  pred->dirpred.twolev2 =
		  bpred_dir_create(class, l1size2, l2size2, shift_width2, xor2);
	...
	unsigned int xor)  	/* history xor address flag */
{
  ...
  case BPred2Level2:
  {
	  if (!l1size || (l1size & (l1size - 1)) != 0)
		  fatal("level-1 size, `%d', must be non-zero and a power of two",
			  l1size2);
	  pred_dir->config.two2.l1size2 = l1size;

	  if (!l2size || (l2size & (l2size - 1)) != 0)
		  fatal("level-2 size, `%d', must be non-zero and a power of two",
			  l2size);
	  pred_dir->config.two2.l2size2 = l2size;

	  if (!shift_width || shift_width > 30)
		  fatal("shift register width, `%d', must be non-zero and positive",
			  shift_width);
	  pred_dir->config.two2.shift_width2 = shift_width;

	  pred_dir->config.two2.xor2 = xor;
	  pred_dir->config.two2.shiftregs2 = calloc(l1size, sizeof(int));
	  if (!pred_dir->config.two2.shiftregs2)
		  fatal("cannot allocate shift register table");

	  pred_dir->config.two2.l2table2 = calloc(l2size, sizeof(unsigned char));
	  if (!pred_dir->config.two2.l2table2)
		  fatal("cannot allocate second level table");
	  ...
	  for (cnt = 0; cnt < l2size; cnt++)
	  {
		  pred_dir->config.two2.l2table2[cnt] = flipflop;
		  flipflop = 3 - flipflop;
	  }
		 ...
		 md_addr_t baddr)		/* branch address */
{
  ...
  switch (pred_dir->class) {
	...
	case BPred2Level2:
	{
		int l1index2, l2index2;

		/* traverse 2-level tables */
		l1index2 = (baddr >> MD_BR_SHIFT) & (pred_dir->config.two2.l1size2 - 1);
		l2index2 = pred_dir->config.two2.shiftregs2[l1index2];
		if (pred_dir->config.two2.xor2)
		{
#if 1
			/* this L2 index computation is more "compatible" to McFarling's
			verison of it, i.e., if the PC xor address component is only
			part of the index, take the lower order address bits for the
			other part of the index, rather than the higher order ones */
			l2index2 = (((l2index2 ^ (baddr >> MD_BR_SHIFT))
				& ((1 << pred_dir->config.two2.shift_width2) - 1))
				| ((baddr >> MD_BR_SHIFT)
					<< pred_dir->config.two2.shift_width2));
#else
			l2index2 = l2index2 ^ (baddr >> MD_BR_SHIFT);
#endif
		}
		else
		{
			l2index2 =
				l2index2
				| ((baddr >> MD_BR_SHIFT) << pred_dir->config.two2.shift_width2);
		}
		l2index2 = l2index2 & (pred_dir->config.two2.l2size2 - 1);

		/* get a pointer to prediction state information */
		p = &pred_dir->config.two2.l2table2[l2index2];
	}
	break;
    case BPred2bit:
					 ...
					 * used on mispredict recovery */
{
  ...
  switch (pred->class) {
      ...
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
	{
	  ...
	  bimod = bpred_dir_lookup (pred->dirpred.twolev2, baddr);
	  ...
	  dir_update_ptr->dir.twolev2 = (*bimod >= 2);
	...
	case BPred2Level2:
		...
		{
			dir_update_ptr->pdir1 =
				bpred_dir_lookup(pred->dirpred.twolev2, baddr);
		}
