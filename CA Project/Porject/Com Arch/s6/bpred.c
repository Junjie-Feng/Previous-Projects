...
/*strategy 6*/
  case BPredStr6:
  {
	  if (!l1size2 || (l1size2 & (l1size2 - 1)) != 0)
		  fatal("level-1 size, `%d', must be non-zero and a power of two",
			  l1size2);
	  pred_dir->config.two2.l1size = l1size2;

	  if (!l2size2 || (l2size2 & (l2size2 - 1)) != 0)
		  fatal("level-2 size, `%d', must be non-zero and a power of two",
			  l2size2);
	  pred_dir->config.two2.l2size = l2size2;

	  if (!shift_width2 || shift_width2 > 30)
		  fatal("shift register width, `%d', must be non-zero and positive",
			  shift_width2);
	  pred_dir->config.two2.shift_width2 = shift_width2;

	  pred_dir->config.two2.xor2 = xor2;
	  pred_dir->config.two2.shiftregs2 = calloc(l1size2, sizeof(int));
	  if (!pred_dir->config.two2.shiftregs2)
		  fatal("cannot allocate shift register table");

	  pred_dir->config.two2.l2table2 = calloc(l2size2, sizeof(unsigned char));
	  if (!pred_dir->config.two2.l2table2)
		  fatal("cannot allocate second level table");

	  /* initialize counters to weakly this-or-that */
	  flipflop = 1;
	  for (cnt = 0; cnt < l2size2; cnt++)
	  {
  ...
  FILE *stream)			/* output stream */
{
  ...
  case BPredStr6:
    fprintf(stream, "pred_dir: %s: 2-bit: %d entries, direct-mapped\n",
      name, pred_dir->config.bimod.size);//************************unmodify
    break;
	     ...
	     FILE *stream)		/* output stream */
{
  switch (pred->class) {
  case BPredComb:
    bpred_dir_config (pred->dirpred.twolev2, "bimod", stream);
  ...
  if (pbtb == NULL)
    {
      ...
      return ((*(dir_update_ptr->pdir1) >= 1) /*strategy 6*/                //******************************************
	      ? /* taken */ 1
	      : /* not taken */ 0);
    }
  else
  ...
  if (dir_update_ptr->pdir1)
    {
       /*strategy 6*/
       switch(pred->class){
         case BPredComb:                    /* combined predictor (McFarling) */
         case BPred2Level:			/* 2-level correlating pred w/2-bit counters */
         case BPred2bit:			/* 2-bit saturating cntr pred (dir mapped) */
           if (taken)
	   {
