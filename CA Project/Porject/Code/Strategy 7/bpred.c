  ...
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  case BPred2bit:
    ...
    /* Strategy 7: 3bit predictor means there are 8 numbers of all the possible value. 
       Because strategy 7 says that the branch will be taken if the sign bit 
       of the accessed count is 0; predict that it will not be taken if the sign
       bit is 1. I assume that -4, -3, -2, -1 will be untaken, and 0, 1, 2, 3 will be taken. 
       so the weakly taken is 0 and the weakly untaken is -1*/
    /* initialize counters to weakly this-or-that */
    flipflop = -1;
